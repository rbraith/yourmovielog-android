package com.rbraithwaite.yourmovielog.data.network

import com.rbraithwaite.yourmovielog.test_utils.ApiTestUtils
import com.rbraithwaite.yourmovielog.test_utils.enqueueResponseFromFile
import com.rbraithwaite.yourmovielog.test_utils.rules.MockWebServerRule
...
class TmdbApiV3Tests {
    ...
    @Test
    fun searchMovie_successTest() = runTest {
        mockWebServerRule.server.enqueueResponseFromFile(RESOURCES_DIR + "SearchMovie.json")

        val result = tmdbApiV3.searchMovies("dummy query")
    ...
    @Test
    fun searchTvShows_successTest() = runTest {
        mockWebServerRule.server.enqueueResponseFromFile(RESOURCES_DIR + "SearchTvShow.json")

        val result = tmdbApiV3.searchTvShows("dummy query")

        assert(result.isSuccess)

        val tvShows = result.getOrThrow()
    ...
    @Test
    fun getTvSeasonDetails_successTest() = runTest {
        mockWebServerRule.server.enqueueResponseFromFile(RESOURCES_DIR + "TvSeasonDetails_FullAppend.json")

        val result = tmdbApiV3.getTvSeasonDetails(123, 4)
        ...
        val tvSeasonDetails = result.getOrThrow()
        assertThat(tvSeasonDetails.id, willBe(72040))
        assertThat(tvSeasonDetails.episodes.size, willBe(10))

        val credits = tvSeasonDetails.aggregateCredits!!
